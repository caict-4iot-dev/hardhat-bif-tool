...
export class AddressCoder extends Coder {
  ...
  encode(writer: Writer, value: string): number {
    ...
    logger.debug(
      `AddressCoder encode address value:${value}, newValue:${newValue}`,
    );
  ...
  decode(reader: Reader): string {
    ...
    const signMap: { [key: string]: string } = { "7a": "z" };
    const encodeMap: { [key: string]: string } = { "73": "s", "74": "t" };

    const newValue = `did:bid:${signMap[signType] || "e"}${encodeMap[encodeType] || "f"}${address}`;
    logger.debug(
      `AddressCoder decode address value:${value}, newValue:${newValue}`,
    );
    return newValue;
  }
}
