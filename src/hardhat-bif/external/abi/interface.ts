...
import { arrayify, BytesLike, concat, hexDataSlice, hexlify, hexZeroPad, isHexString } from "../bytes";
import { id } from "../id";
...
export class Interface {
    ...
    _encodeParams(params: ReadonlyArray<ParamType>, values: ReadonlyArray<any>): string {
        logger.debug(`_encodeParams:${JSON.stringify(params)}, values:${JSON.stringify(values)}`)
    ...
    encodeFilterTopics(eventFragment: EventFragment | string, values: ReadonlyArray<any>): Array<string | Array<string>> {
        ...
        const encodeTopic = (param: ParamType, value: any): string => {
            ...
            if (param.type === "address") { 
                value = this._abiCoder.encode( [ "address" ], [ value ]); 
            }
            logger.debug(`encodeTopic: param:${JSON.stringify(param)}, value:${JSON.stringify(value)}`)
