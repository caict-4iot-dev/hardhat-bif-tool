...
async function resolveBidAddress(
...
function buildCall(
  ...
  return async function (...args: Array<any>): Promise<any> {
    ...
    try {
      logger.debug(
        `buildCall fragment:${JSON.stringify(fragment)}, result:${result}`,
      );
    ...
    if (!this._deployedPromise) {
      ...
      if (this.deployTransaction) {
        this._deployedPromise = this.deployTransaction.wait().then((result) => {
          if (result.status === 0) {
            console.log(
              `transaction hash:${this.deployTransaction.hash} execute success`,
            );
            return this;
          } else {
            console.log(
              `transaction hash:${this.deployTransaction.hash} execute failed. errcode:${result.status}, errdesc:${result.logsBloom}`,
            );
            return null;
          }
        });
