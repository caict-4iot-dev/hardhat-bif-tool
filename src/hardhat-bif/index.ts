import { extendConfig, extendProvider, extendEnvironment } from "hardhat/config";
...
extendConfig((config, userConfig) => {
    ...
    for (const networkName of Object.keys(config.networks)) {
        config.networks[networkName].bifNet = userConfig.networks?.[networkName]?.bifNet ?? false;
        config.networks[networkName].bifAccounts = userConfig.networks?.[networkName]?.bifAccounts ?? [];
        ...
        if (config.networks[networkName].bifNet) {
            ...
            config.networks[networkName].bifAccounts.filter(account => {  
                if (!account.startsWith('pri')) {  
                    console.log(
                        chalk.yellow(
                          `Warning: account ${account} pri not exist`
                        )
                    );
                }  
            });  
    ...
    if (!existBif) {
        console.log(
            chalk.yellow(
              `Warning: bif net config not exist`
            )
        );
    }
});
...
extendProvider(async (provider, config, network) => {
    ...
    if (config.networks[network].bifNet) {
        ...
        if (net.url === undefined || net.bifAccounts === undefined) {
            console.log(
                chalk.yellow(
                  `Warning: bif net config not exist`
                )
...
extendEnvironment((hre) => {
    hre.bif = lazyObject(() => {
      const { HardhatBifProvider } = require("./internal/hardhat-bif-provider") as {
        HardhatBifProvider: typeof HardhatBifProviderT;
      };
  
      const provider = new HardhatBifProvider(
