...
import {
  NotExistAccountPrivateKeyError,
  ProviderMethodError,
  MethodInputError,
  InvalidBifPrivateKeyError,
} from "./errors";
import { EventEmitter } from "events";

const BIFCoreSDK = require("@caict/bif-core-sdk");

export class LocalBifAccountsProvider
  ...
  implements EIP1193Provider
{
  ...
  public async request(args: RequestArguments): Promise<unknown> {
    ...
    if (object && typeof object[methodName] === "function") {
      if (params.length === 0) return object[methodName]();
      else return object[methodName](...params);
    } else {
  ...
  private _initializePrivateKeys(localBifAccountsPrivateKeys: string[]) {
    ...
    for (const pk of privateKeys) {
      try {
        const pri = getEncPublicKey(pk);
        const address: string = publicToAddress(pri);
        this._addressToPrivateKey.set(address, pk);
      } catch {
        throw new InvalidBifPrivateKeyError(pk);
      }
    }
  }
